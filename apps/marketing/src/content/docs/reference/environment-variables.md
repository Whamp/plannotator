...
| `PLANNOTATOR_PUBLIC_URL` | unset | Full HTTP(S) origin advertised for a remote session behind a reverse proxy, for example `https://plannotator.example.com`. Plannotator uses it as-is instead of appending the local runtime port. Origin only: credentials, paths, query strings, fragments, and non-HTTP(S) schemes warn once on stderr and fall back to `PLANNOTATOR_URL_HOST` or `localhost`. Strictly display-only and remote-only: it never changes the bind interface or same-machine agent API URL. Can also be set via `~/.plannotator/config.json` (`{ "publicUrl": "https://plannotator.example.com" }`); the env var takes precedence, and setting it to an empty value suppresses the config key. |
| `PLANNOTATOR_URL_HOST` | unset (`localhost`) | Display-only hostname for advertised session URLs, e.g. a Tailscale MagicDNS name or tailnet IP, so remote-mode links are reachable from another device instead of `http://localhost:<port>`. Host only: bare hostname, IPv4, or bracketed IPv6 such as `[fd7a::1]` — the runtime-chosen port is always appended, and a value carrying a scheme, port, path, or whitespace warns on stderr and falls back to `localhost`. Ignored when `PLANNOTATOR_PUBLIC_URL` is set. Strictly display-only and remote-only: it never changes which interface the server binds (that stays governed by `PLANNOTATOR_REMOTE`), and a local session ignores the override — the advertised URL stays `localhost`, with a stderr warning to set `PLANNOTATOR_REMOTE=1`. Can also be set via `~/.plannotator/config.json` (`{ "urlHost": "host" }`); the env var takes precedence, and setting it to an empty value suppresses a config-file `urlHost`. |
...
| `PLANNOTATOR_GUIDE_VIEWER_URL` | `https://guides.show/v1/` | Base URL of the viewer that downloaded portable guides pin. Must be `https:` (or `http:` on localhost). Also `--viewer-url` on `plannotator guide export`. |
| `PLANNOTATOR_GUIDE_SHARE_URL` | `https://guides.show` | Base URL of the guide host that Guided Review share links are created on. Set this to your own deployment of the `apps/guides-show` Cloudflare Worker. Also `{ "guideShareUrl": "..." }` in `~/.plannotator/config.json`; the env var takes precedence. |
...
| `PLANNOTATOR_FEEDBACK_HISTORY` | enabled | Set to `0` or `false` to stop archiving the feedback you submit under `~/.plannotator/feedback/` (or `PLANNOTATOR_DATA_DIR`). When enabled, every plan decision, code review submission, and annotate submission appends one JSON record — plus a readable markdown copy when it carries content — so a review survives an agent that timed out or a terminal you closed. It stores your own review text, the document and code excerpts it quotes, and annotation metadata; it never stores the diff itself, and nothing prunes the directory, so delete `~/.plannotator/feedback/` (or one project subdirectory) to forget. Annotate sessions also honor `PLANNOTATOR_ANNOTATE_HISTORY=0`, which keeps them fully stateless. A review posted straight to GitHub or GitLab is delivered to the platform and is not archived locally. Can also be set via `~/.plannotator/config.json` (`{ "feedbackHistory": false }`); the env var takes precedence. |
| `PLANNOTATOR_ANNOTATE_HISTORY` | enabled | Set to `0` or `false` to keep annotate sessions fully stateless: no per-file version history (so no annotate version diff), no durable submitted-feedback records, and no feedback-archive records from any annotate surface. Can also be set via `~/.plannotator/config.json` (`{ "annotateHistory": false }`); the env var takes precedence. |
...
| `PLANNOTATOR_INSTALL_CALLDIFF` | off | Set to `1` / `true` / `yes` to install the optional pinned, pruned CallDiff core (about 5 MB on macOS arm64, Node.js 22+). It is not installed by default; normally, enabling Call flow in-app consents to a background install of core and only the language packs required by each review. Equivalent to `--with-call-flow` (PowerShell: `-WithCallFlow`) or `{ "installCallFlow": true }`; flag > env var > config. Read by the install scripts only. |
