...
| `PLANNOTATOR_PUBLIC_URL` | unset | Full HTTP(S) origin advertised in remote mode behind a reverse proxy. The local runtime port is not appended. Binding is unaffected and local sessions ignore the override. Can also be set via `~/.plannotator/config.json` (`{ "publicUrl": "https://plannotator.example.com" }`). |
| `PLANNOTATOR_URL_HOST` | unset (`localhost`) | Display-only hostname for advertised session URLs in remote mode (e.g. a Tailscale MagicDNS name). The runtime-chosen port is always appended; binding is unaffected and local sessions ignore the override. Ignored when `PLANNOTATOR_PUBLIC_URL` is set. Can also be set via `~/.plannotator/config.json` (`{ "urlHost": "host" }`). |
